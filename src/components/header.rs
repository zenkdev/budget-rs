...
pub fn Header() -> Html {
    ...
    <header class="flex flex-col sm:flex-row items-start sm:items-center justify-between whitespace-nowrap border-b border-solid border-primary/30 px-4 sm:px-10 py-3 text-glow">
        <div class="flex items-center gap-4 text-primary mb-2 sm:mb-0">
            ...
            <h2 class="text-primary text-lg font-bold leading-tight tracking-[-0.015em]">{"V.B.S. (VAULT BUDGETING SYSTEM) v1.0"}</h2>
        </div>
        <div class="flex flex-1 sm:justify-end gap-4 sm:gap-8">
            <div class="flex items-center gap-4 sm:gap-9">
                <a class="text-primary text-sm font-medium leading-normal hover:text-white" href="#">{"[STATUS]"}</a>
                <a class="text-primary text-sm font-medium leading-normal hover:text-white" href="#">{"[DATA]"}</a>
                <a class="text-primary text-sm font-medium leading-normal hover:text-white" href="#">{"[SETTINGS]"}</a>
